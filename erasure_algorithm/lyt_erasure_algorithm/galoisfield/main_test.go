...
func NewErasure(ctx context.Context, dataBlocks, parityBlocks int, blockSize int64) (e Erasure, err error) {
	...
	if dataBlocks <= 0 || parityBlocks <= 0 {
		return e, ErrInvShardNum
	}

	if dataBlocks+parityBlocks > 256 {
		return e, ErrMaxShardNum
	}
	...
	var enc Encoder
	var once sync.Once
	e.encoder = func() Encoder {
		once.Do(func() {
			fmt.Println("Creating new encoder")
			fmt.Println(dataBlocks, parityBlocks, blockSize)
			e, err := Raid6New(dataBlocks, parityBlocks)
			if err != nil {
...
func erasureSelfTest() {
	...
	// 	}
	// }
	testConfigs = append(testConfigs, [2]uint8{2, 2})
	fmt.Println("Running erasure self-test... ")
	fmt.Println("Total test configurations:", len(testConfigs))
	fmt.Println(testConfigs)
